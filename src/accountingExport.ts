...
export function renderAccountingHtml(report: AccountingReport, company: CompanySettings) {
  const monthlyRows = report.months
    .map(
      (month) =>
        `<tr><td>${esc(month.label)}</td><td class="num">${currency(month.revenueHt)}</td><td class="num">${currency(month.purchasesHt)}</td><td class="num">${currency(month.operatingExpensesHt)}</td><td class="num">${currency(month.marginAmount)}</td><td class="num">${currency(month.netProfit)}</td><td class="num">${percent(month.marginRate)}</td></tr>`
    )
    .join("");
  ...
  @page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#26332f;margin:0;font-size:10px}header{display:flex;justify-content:space-between;border-bottom:3px solid #1f5f52;padding-bottom:10px;margin-bottom:14px}h1{font-size:22px;margin:0;color:#17483f}h2{font-size:14px;color:#17483f;margin:18px 0 7px}.meta{text-align:right;color:#65746f}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.kpi{border:1px solid #d8dfdc;border-radius:5px;padding:9px;background:#f5f8f6}.kpi span{display:block;color:#65746f;text-transform:uppercase;font-size:8px;font-weight:700}.kpi strong{display:block;font-size:15px;margin-top:5px;color:#17483f}table{width:100%;border-collapse:collapse}th{background:#17483f;color:#fff;text-align:left;padding:6px}td{padding:5px 6px;border-bottom:1px solid #e2e7e4}.num{text-align:right;white-space:nowrap}.detail{page-break-before:always}.note{margin-top:10px;color:#65746f;font-size:9px}</style></head><body>
