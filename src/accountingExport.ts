...
export function renderAccountingHtml(report: AccountingReport, company: CompanySettings) {
  ...
  const profitAfterVatBeforeTax = Math.round((profitBeforeVatAndTax - report.vatBalance + Number.EPSILON) * 100) / 100;
  const finalNetProfit = Math.round((profitAfterVatBeforeTax - report.estimatedCorporateTax + Number.EPSILON) * 100) / 100;
  ...
  <h2>Résultat final estimé</h2><table class="resultTable"><tbody><tr><th>Résultat avant TVA et impôts</th><td class="num">${currency(profitBeforeVatAndTax)}</td></tr><tr><th>${vatBalanceLabel}</th><td class="num">${currency(report.vatBalance)}</td></tr><tr><th>Résultat après TVA, avant impôts</th><td class="num">${currency(profitAfterVatBeforeTax)}</td></tr><tr><th>Impôt société estimé</th><td class="num">${currency(report.estimatedCorporateTax)}</td></tr><tr><th>Net final estimé après TVA et impôts</th><td class="num resultFinal">${currency(finalNetProfit)}</td></tr></tbody></table>
  ...
  <p class="note">Le solde TVA correspond à la TVA collectée sur les ventes moins la TVA déductible des dépenses et factures fournisseur enregistrées. Le résultat avant IS correspond à la marge sur ventes diminuée des dépenses enregistrées. Toute charge non saisie dans Devix reste exclue du calcul.</p></body></html>`;
}
